...
class Net(nn.Module):
    def __init__(self):
        ...
        self.conv_pointwise = nn.Conv2d(in_channels=20 * 32, out_channels=32, kernel_size=1, stride=1, padding=0,
                                         bias=True)
        self.conv_merge = nn.Conv2d(in_channels=96, out_channels=64, kernel_size=3, stride=1, padding=1, bias=True)
    ...
    def forward(self, lr, label):
        #show_features(x, 0, 3)

        for i in range(20):
            if i ==0:
                mask = label[:,i:i+1,:,:].repeat(1,32,1,1)
            else:
                mask = torch.cat((mask,label[:,i:i+1,:,:].repeat(1,32,1,1)),1)
        mask = mask.detach()
        out = lr.repeat(1,20,1,1)

        out = self.conv_clswise(out)
        residual = out
