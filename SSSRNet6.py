...
class Net(nn.Module):
    def __init__(self):
        ...
        self.upscale4x = nn.Sequential(
            nn.Conv2d(in_channels=21*64, out_channels=21 * 128, kernel_size=3, stride=1, padding=1,
                                          bias=True, groups=21),
            nn.PixelShuffle(2),
            nn.LeakyReLU(0.2, inplace=True),
            nn.Conv2d(in_channels=21 * 32, out_channels=21 * 128, kernel_size=3, stride=1, padding=1,
                      bias=True, groups=21),
            nn.PixelShuffle(2),
            nn.LeakyReLU(0.2, inplace=True),
        )

        self.conv_pointwise = nn.Conv2d(in_channels=21 * 32, out_channels=32, kernel_size=1, stride=1, padding=0,
                                         bias=True)
        self.conv_output = nn.Conv2d(in_channels=32, out_channels=3, kernel_size=9, stride=1, padding=4, bias=True)
