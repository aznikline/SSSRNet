...
from visualization import show_features_test
...
class _Clswise_Residual_Block(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(in_channels=32*21, out_channels=32*21, kernel_size=3, stride=1, padding=1, bias=True, groups=21)
        # self.bn1 = nn.BatchNorm2d(64)
        self.relu = nn.LeakyReLU(0.2, inplace=True)
        self.conv2 = nn.Conv2d(in_channels=32*21, out_channels=32*21, kernel_size=3, stride=1, padding=1, bias=True, groups=21)
        # self.bn2 = nn.BatchNorm2d(64)
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.conv_clswise = nn.Conv2d(in_channels=63, out_channels=21*32, kernel_size=5, stride=1, padding=2, bias=True, groups=21)
        self.relu = nn.LeakyReLU(0.2, inplace=True)

        self.clswise_residual = self.make_layer(_Clswise_Residual_Block, 4)
        self.residual = self.make_layer(_Residual_Block, 4)

        self.conv_clswise_mid = nn.Conv2d(in_channels=21*32, out_channels=21*32, kernel_size=3, stride=1, padding=1, bias=True, groups=21)
        ...
        self.conv_pointwise = nn.Conv2d(in_channels=21 * 32, out_channels=64, kernel_size=1, stride=1, padding=0,
                                         bias=True)
