...
class Net(nn.Module):
    def __init__(self):
        ...
        self.conv_clswise_in = nn.Conv2d(in_channels=63, out_channels=21 * 64, kernel_size=5, stride=1, padding=2,
                                      bias=True, groups=21)
        self.conv_pointwise_in = nn.Conv2d(in_channels=21 * 64, out_channels=64, kernel_size=1, stride=1, padding=0,
                                         bias=True)
        self.relu = nn.LeakyReLU(0.2, inplace=True)
    ...
    def forward(self, x, mask):
        out = self.conv_clswise_in(x)
        residual_cls = out
        out =self.conv_pointwise_in(out)
        residual = out
        ...
        mask_selected = torch.add(mask_selected, residual_cls)
