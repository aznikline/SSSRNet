...
def main():
    print('Modified Time: 20:40 12/15/2017')
    ...
    root_dir = '/tmp4/hang_data/VOCdevkit/VOC2012/VOC_train_label320_HDF5'
...
def train(training_data_loader, optimizer, model, model_pretrained, criterion, epoch, gpuid):
    ...
    for iteration, batch in enumerate(training_data_loader, 1):
        ...
        is_show = False
        if is_show == True:
            output0 = label.cpu().data[0].numpy()
...
def save_checkpoint(model, epoch):
    model_out_path = "model/" + "SSSRNet1_320_epoch_{}.pth".format(epoch)
