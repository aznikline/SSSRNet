...
parser.add_argument("--clip", type=float, default=2000, help="Clipping Gradients. Default=0.1")
...
def main():
    print("SSSRNet training from scratch on VOC 160*160 patches (ignoring bg labels).")
...
def train(training_data_loader, optimizer, model, model_pretrained, criterion, epoch, gpuid):
    ...
    for iteration, batch in enumerate(training_data_loader, 1):
        ...
        #=======label transform h*w to 21*h*w=======#
        label = Variable(torch.from_numpy(label_pro[:,1:,:,:]).float())
...
def save_checkpoint(model, epoch):
    model_out_path = "model/" + "SSSRNet2_160_epoch_{}.pth".format(epoch)
