...
from SSSRNet5_baseline2_concat import Net
...
def main():
    print("SSSRNet5_baseline2_concate (high lr) scratch training on VOC 160*160 patches.")
...
def train(training_data_loader, optimizer, model, model_pretrained, criterion, epoch, gpuid):
    ...
    for iteration, batch in enumerate(training_data_loader, 1):
        ...
        if is_show == True:
            ...
            label_heatmap = label.cpu().data[0].view(20, 1, input.data[0].size(1), input.data[0].size(2))
            label_heatmap = torchvision.utils.make_grid(label_heatmap)
            label_heatmap = label_heatmap.numpy().transpose((1, 2, 0))
            images_cls = input_cls.cpu().data[0].view(20, 3, input.data[0].size(1), input.data[0].size(2))
...
def save_checkpoint(model, epoch):
    model_out_path = "model/" + "SSSRNet5_baseline2_concate_lr_scratch_{}.pth".format(epoch)
