...
from SSSRNet5_finetune import Net
from dataset_hdf5 import DatasetFromHdf5_OLD
...
def main():
    print("SSSRNet5 finetune training on VOC 160*160 patches.")
    ...
    model = Net()
    print('Parameters: {}'.format(vl.get_n_params(model)))
    ...
    root_dir = '/tmp4/hang_data/VOCdevkit/VOC2012/VOC_train_hrlabel160_HDF5_0131'
...
def train(training_data_loader, optimizer, model, model_pretrained, criterion, epoch, gpuid):
    ...
    for iteration, batch in enumerate(training_data_loader, 1):
        ...
        #vl.show_image(input)
        #vl.show_image(target)
        #vl.show_label(label.data[:], is_label=False)
        #vl.show_features(label, batch=0, num_chns=1)
        ...
        #vl.show_features(input_cls, batch=0, num_chns=3)
        input_cls = input_cls.cuda(gpuid)
        output = model(input_cls, input)
...
def save_checkpoint(model, epoch):
    model_out_path = "model/" + "SSSRNet5_finetune160_epoch_{}.pth".format(epoch)
