...
class Net(nn.Module):
    def __init__(self):
        ...
        self.conv_pointwise = nn.Conv2d(in_channels=20 * 3, out_channels=3, kernel_size=1, stride=1, padding=0,
                                         bias=True)
    ...
    def forward(self, x, label):
        ...
        for i in range(20):
            ...
            mask_selected = torch.mul(mask, out_fg[:, i*3:(i + 1)*3, :, :])
            if i == 0:
                out_merge = mask_selected
                out_cat = mask_selected
            else:
                out_merge = torch.add(out_merge, mask_selected)
                out_cat = torch.cat((out_cat, mask_selected),1)

        out = self.upscale4x(out)
        out = self.conv_output(out)

        output_mask = (out_merge == 0)
        out = out[output_mask]
        out = torch.add(out, out_merge)
        return out, out_cat
