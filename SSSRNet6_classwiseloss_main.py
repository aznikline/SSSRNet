...
from SSSRNet6_classwiseloss import Net
...
def train(training_data_loader, optimizer, model, model_pretrained, criterion, epoch, gpuid):
    ...
    for iteration, batch in enumerate(training_data_loader, 1):
        ...
        for i in range(21):
            ...
            mask_selected = torch.mul(mask_in, target)
            if i == 0:
                target_cls = mask_selected
            else:
                target_cls = torch.cat((target_cls, mask_selected), dim=1)
        ...
        output, output_cls = model(input, mask)
        ...
        loss_cls = criterion(output_cls, target_cls) / opt.batchSize
        loss = loss+ loss_cls* 10
        avgloss += loss.data[0]
...
def save_checkpoint(model, epoch):
    model_out_path = "model/" + "SSSRNet4_local160_epoch_{}.pth".format(epoch)
