...
DATA_DIRECTORY = '/tmp5/hang_data/SS_SR_data/'
...
def main():
    ...
    testloader = data.DataLoader(VOCDataTestSet(DATA_DIRECTORY, DATA_LIST_PATH, crop_size=(120, 120), mean=IMG_MEAN),
    ...
    for epoch in range(opt.start_epoch, opt.nEpochs + 1):
        ...
        #for index in range(0, files_num):
            ...
            #training_data_loader = DataLoader(dataset=train_set, num_workers=opt.threads, batch_size=opt.batchSize,
                                            #shuffle=True)
        avgloss = train(testloader, optimizer, deeplab_res, model, mid, criterion, epoch, gpuid)
        if epoch % 2 ==0 :
            save_checkpoint(model, epoch)
...
def save_checkpoint(model, epoch):
    model_out_path = "model/" + "model_epoch_scratch{}.pth".format(epoch)
